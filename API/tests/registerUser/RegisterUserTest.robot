...
CT01: Register a new user successfully
    ...
    Create a new user
    Register a new user by Api    ${PATH.user}    ${FAKER_NAME_USER}    ${FAKER_NAME_USER}@testqa.com    ${FAKER_PASSWORD_USER}
...
CT02: Register a new user unsuccessfully
    [Tags]    regression 
    Register a new user by Api    ${PATH.user}    ${default_name}    ${default_email}    ${default_password}
...
CT06: Change a specific user successfully
    [Tags]    regression
    Create a new user
    Register a new user by Api    ${PATH.user}    ${FAKER_NAME_USER}    ${FAKER_NAME_USER}@qa.com    ${FAKER_PASSWORD_USER}
    Set Test Variable             ${UserId}    ${response.json()['_id']}
    Change a new user by Api      ${PATH.user}    ${UserId}    ${other_name}    ${other_email}    ${other_password}
    Should Be Equal As Numbers    ${Response.status_code}              200
    Should Be Equal As Strings    ${Response.json()['message']}        Registro alterado com sucesso
    Delete user by id             ${PATH.user}    ${UserId}
